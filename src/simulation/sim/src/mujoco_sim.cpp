...
Mujoco_Sim::Mujoco_Sim()
{
    ...
    robot_state.motor_state.resize(16);
    robot_command.motor_command.resize(16);
    mujoco_torque.resize(16);
...
void Mujoco_Sim::joint_Command() // 传入关节控制指令
{
    if (mj_data )
    {
        for (int i = 0; i < 16; i++)
        {
            mujoco_torque[i] = (robot_command.motor_command[i].q - mj_data->sensordata[i]) * robot_command.motor_command[i].kp + (robot_command.motor_command[i].dq - mj_data->sensordata[i + 16]) * robot_command.motor_command[i].kd + robot_command.motor_command[i].tau; // 计算关节力矩
...
void Mujoco_Sim::joint_State() // 获取关节状态
{
    if (mj_data && mj_model)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            robot_state.motor_state[i].dq = mj_data->sensordata[i + 16];      // 关节速度
            robot_state.motor_state[i].tau_est = mj_data->sensordata[i + 32]; // 关节力矩
        }
        // imu的四元数、陀螺仪、加速度计
        for (int i = 0; i < 4; i++)
        {
            robot_state.imu.quaternion[i] = mj_data->sensordata[48 + i]; // 四元数
        }
        for (int i = 0; i < 3; i++)
        {
            robot_state.imu.gyroscope[i] = mj_data->sensordata[52 + i];     // 陀螺仪
            robot_state.imu.accelerometer[i] = mj_data->sensordata[55 + i]; // 加速度计
        }
    }
}
