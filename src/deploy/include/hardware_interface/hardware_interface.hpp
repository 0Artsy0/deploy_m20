...
public:
    robot_control_param param; // 机器人参数
    ...
    hardware_interface(const std::string &hardware_name=""):param()
    {
        this->hardware_name=hardware_name;
        
        pos.resize(param.dof_nums,0.0f);
        vel.resize(param.dof_nums,0.0f);

        cmd_pos.resize(param.dof_nums,0.0f);
        cmd_vel.resize(param.dof_nums,0.0f);
        cmd_torque.resize(param.dof_nums,0.0f);
        kp.resize(param.dof_nums,0.0f);
        kd.resize(param.dof_nums,0.0f);
